...
public class MouseHandler
{
    ...
    private unsafe void OnMove(IntPtr handle, double x, double y)
    {
    ...
    private unsafe void OnPress(IntPtr handle, MouseButton button, InputAction action, KeyModifiers mods)
    {
        if (handle != Game.Instance.Window.Handle) return;
    ...
    private unsafe void OnScroll(IntPtr handle, double x, double y)
    {
        if (handle != Game.Instance.Window.Handle) return;

    }
    
    private unsafe void OnDrop(IntPtr handle, List<string> paths)
    {
        if (handle != Game.Instance.Window.Handle) return;
