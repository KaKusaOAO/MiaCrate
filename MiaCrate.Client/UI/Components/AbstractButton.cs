...
public abstract class AbstractButton : AbstractWidget
{
    ...
    protected override void RenderWidget(GuiGraphics graphics, int mouseX, int mouseY, float f)
    {
        ...
        RenderString(graphics, game.Font, k.WithAlpha((byte) Math.Ceiling(Alpha * 255)));
    }

    public void RenderString(GuiGraphics graphics, Font font, Argb32 color) => 
        RenderScrollingString(graphics, font, TextMargin, color);
}