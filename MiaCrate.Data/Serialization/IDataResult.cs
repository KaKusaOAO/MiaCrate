...
public interface IDataResult<T> : IDataResult, IApp<IDataResult.Mu, T>
{
    ...
    public IEither<T, IPartialResult> Get();
    public IOptional<T> Result => Get().Left;
    public IOptional<IPartialResult> Error => Get().Right;

    public IDataResult<T> SetPartial(T partial)
    {
        return new DataResult<T>(
            Get().SelectRight(r =>
                (IPartialResult) new DataResult<T>.PartialResult(r.MessageFunc, Optional.Of(partial))
            ), Lifecycle
        );
    }
    ...
    public IDataResult<T> SetLifecycle(Lifecycle lifecycle) => new DataResult<T>(Get(), lifecycle);

    public IDataResult<TOut> Apply2Stable<TSecond, TOut>(Func<T, TSecond, TOut> func, IDataResult<TSecond> second)
    {
...
public sealed class DataResultInstance : IApplicative<IDataResult.Mu, DataResultInstance.Mu>
{
    ...
    public IApp<IDataResult.Mu, TResult> Ap<TArg, TResult>(IApp<IDataResult.Mu, Func<TArg, TResult>> func,
        IApp<IDataResult.Mu, TArg> arg) =>
